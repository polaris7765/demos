...
namespace HighFlyers
{
    public class Main : MonoBehaviour
    {
        ...
        private void InitAllItems(int startIndex)
        {
        ...
        private void CreateItems(GComponent container, List<OptionsItem> items)
        {
            ...
            //_itemStates = new Dictionary<string, Common.ButtonState>();
            ...
            for (int i = 0; i < _totalIndex; i++)
            {
                ...
                //_itemStates.Add(i.ToString(), Common.ButtonState.Disable);
        ...
        private void AfterClickedItem(int index, int clickIndex)
        {
            ...
            Common.AnswerItem answerItem = new Common.AnswerItem() {index = index, col = item.colIndex, row = item.rowIndex};
        ...
        {
            item.GetObject.asCom.GetTransition(state.ToString() + dir).Play();
        }
            ...
            for (int i = 0; i < count; i++)
            {
                if (i > 0)
                {
                    ...
                    SetItemAnimation(obj,
                        GetArrowDirection(new Common.AnswerItem() {row = item1.rowIndex, col = item1.colIndex}, new Common.AnswerItem() {row = item2.rowIndex, col = item2.colIndex}));
                    if (i == count - 1)
                    {
